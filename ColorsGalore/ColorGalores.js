(function (Scratch) {
    ...
    function parseRGB(input) {
        ...
        const hslMatch = text.match(/hsla?\(\s*([\d.]+)\s*,\s*([\d.]+)%\s*,\s*([\d.]+)%\s*(?:,\s*[\d.]+\s*)?\)/i);
        if (hslMatch) {
            return hslToRGB(
        ...
        const cmykMatch = text.match(/cmyk\(\s*([\d.]+)%?\s*,\s*([\d.]+)%?\s*,\s*([\d.]+)%?\s*,\s*([\d.]+)%?\s*\)/i);
        if (cmykMatch) {
            return cmykToRGB([
        ...
        const oklchMatch = text.match(/oklch\(\s*([\d.]+)%?\s+([\d.]+)\s+([\d.]+)\s*\)/i);
        if (oklchMatch) {
            return oklchToRGB(
        ...
        const pmsExact = PMS_COLORS.find(color => color.name.toLowerCase() === text.toLowerCase());
        if (pmsExact) return pmsExact.rgb;

        const pmsShort = text.match(/^pms\s+(.+)$/i);
        if (pmsShort) {
            const match = PMS_COLORS.find(color =>
                color.name.toLowerCase().includes(pmsShort[1].toLowerCase())
            );
    ...
    function hslToRGB(h, s, l) {
        ...
        return [r * 255 + m * 255, g * 255 + m * 255, b * 255 + m * 255];
    }
    ...
    function rgbToOKLCH(rgb) {
        ...
        const bVal = 0.0259040371 * l3 + 0.7827717662 * m3 - 0.808675766 * s3;
    ...
    function oklchToRGB(L, C, H) {
        ...
        const s3 = L - 0.0894841775 * a - 1.291485548 * bVal;
    ...
    class ColorGalores {
        ...
        getColorValue({ TYPE, COLOR }) {
            return formatColor(TYPE, parseRGB(COLOR));
        }

        convertColor({ FROM, COLOR, TO }) {
            const rgb = parseRGB(COLOR);
            return formatColor(TO, rgb);
        }
    }

    Scratch.extensions.register(new ColorGalores());
})(Scratch);